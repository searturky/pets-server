...
type Repository interface {
	...
	// FindByID 根据ID查找用户道具
	FindByID(ctx context.Context, id int64) (*UserItem, error)

	// FindByUserAndItem 根据用户ID和道具ID查找
	FindByUserAndItem(ctx context.Context, userID int64, itemID int) (*UserItem, error)

	// FindByUserID 获取用户所有道具
	FindByUserID(ctx context.Context, userID int64) ([]*UserItem, error)

	// FindByUserAndType 获取用户某类型的所有道具
	FindByUserAndType(ctx context.Context, userID int64, itemType ItemType) ([]*UserItem, error)
...
type DecorationRepository interface {
	...
	FindByPetID(ctx context.Context, petID int64) ([]*PetDecoration, error)

	// FindByPetAndSlot 获取宠物某槽位的装饰
	FindByPetAndSlot(ctx context.Context, petID int64, slot string) (*PetDecoration, error)
